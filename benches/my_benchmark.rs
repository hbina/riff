...
fn test_chunk_lazy(_: ()) {
    {
        let chunk_root = lazy::Chunk::from_path("test_assets/set_4.riff").unwrap();
        {
        ...
        {
            let list_1 = chunk_root.iter().unwrap().next().unwrap().unwrap();
        ...
        {
            let list_1 = chunk_root.iter().unwrap().skip(1).next().unwrap().unwrap();
    ...
    {
        let chunk_root = lazy::Chunk::from_path("test_assets/set_3.riff").unwrap();
        {
    ...
    {
        let chunk_root = lazy::Chunk::from_path("test_assets/set_2.riff").unwrap();
    ...
    {
        let chunk_root = lazy::Chunk::from_path("test_assets/set_1.riff").unwrap();
...
fn test_chunk_eager(_: ()) {
    {
        let chunk_root = eager::Chunk::from_path("test_assets/set_4.riff").unwrap();
        {
            assert_eq!(chunk_root.payload_len().unwrap(), 102);
            assert_eq!(chunk_root.id().unwrap().as_bytes(), RIFF_ID);
            assert_eq!(chunk_root.chunk_type().unwrap().as_bytes(), b"smpl");
    ...
    {
        let chunk_root = eager::Chunk::from_path("test_assets/set_3.riff").unwrap();
        {
            assert_eq!(chunk_root.payload_len().unwrap(), 100);
            assert_eq!(chunk_root.id().unwrap().as_bytes(), RIFF_ID);
            assert_eq!(chunk_root.chunk_type().unwrap().as_bytes(), b"smpl");
    ...
    {
        let chunk_root = eager::Chunk::from_path("test_assets/set_2.riff").unwrap();
        assert_eq!(chunk_root.payload_len().unwrap(), 24);
        assert_eq!(chunk_root.id().unwrap().as_bytes(), b"RIFF");
        assert_eq!(chunk_root.chunk_type().unwrap().as_bytes(), b"smpl");
        let expected_content = vec![(b"tst1", vec![255]), (b"tst2", vec![238])];
    ...
    {
        let chunk_root = eager::Chunk::from_path("test_assets/set_1.riff").unwrap();
        assert_eq!(chunk_root.payload_len().unwrap(), 14);
        assert_eq!(chunk_root.id().unwrap().as_bytes(), b"RIFF");
        assert_eq!(chunk_root.chunk_type().unwrap().as_bytes(), b"smpl");
        let expected_content = vec![vec![255]];
...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("lazy sets => ", |b| {
        b.iter(|| test_chunk_lazy(black_box(())))
    });
    c.bench_function("eager sets => ", |b| {
        b.iter(|| test_chunk_eager(black_box(())))
    });
}
